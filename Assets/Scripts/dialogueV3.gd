...
func _show_next_line():
	...
	state = State.READING
	tween = create_tween()
	tween.tween_property(dialogue_text, "visible_ratio", 1.0, dialogue_text.text.length() * local_speed)
	handle_tween_finish()

func handle_tween_finish() -> void:
...
func _process(_delta):
	if Input.is_action_just_pressed("ui_accept"):
		match state:
			State.READING:
				if skip_allowed:
					# يمكنك تفعيل هذا الجزء إذا أردت تجاوز فوري عند الضغط
					# tween.kill()
...
func _on_image_timer_timeout():
	...
	if image_index >= image_list.size():
		image_index = 0  # أو image_timer.stop() لو أردت أن تتوقف عند آخر صورة
