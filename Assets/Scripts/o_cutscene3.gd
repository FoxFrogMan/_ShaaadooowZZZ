...
@onready var ost1 := $farmOst1
@onready var ost2 := $farmOst2

var state := 1

func _ready():
	ui.modulate.a = 0
	#TranslationServer.set_locale("en") #its already set to "en" by SaveLoader (global node)
	_init_scene()
	begin_cutscene()
...
func execute_state():
	match state:
		...
		5:
			...
			await t.finished
			await _run_dialogue("cutscene_6")
			state = 6
			execute_state()
		6:
			omyx.play("up")
			var t = create_tween()
			t.tween_property(omyx, "position", Vector2(111, 240), 3)
			await t.finished
			omyx.stop()
			omyx.animation = "up"
			omyx.play("following")
...
func _finalize():
	...
	screen.modulate.a = 0
	var t = create_tween()
	t.tween_property(screen, "modulate:a", 1.0, 2)
	await t.finished
	SaveLoader.musicSave1 = ost2.get_playback_position() #variable for storing music playback
